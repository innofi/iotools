...
#define LIGHTS_LED_TIMER_BIT               (LEDC_TIMER_13_BIT)
#define LIGHTS_LED_FULL_DUTY               ((1 << LIGHTS_LED_TIMER_BIT) - 1)
...
static light_handle_t g_led_handle = NULL;
static bool g_led_blink_flag       = false;
...
esp_err_t led_set_rgb(uint8_t red, uint8_t green, uint8_t blue)
{
    ...
    ret = iot_light_fade_with_time(g_led_handle, CHANNEL_ID_RED, red * LIGHTS_LED_FULL_DUTY / 255, 0);
    MDF_ERROR_CHECK(ret < 0, ret, "iot_light_fade_with_time, ret: %d", ret);

    ret = iot_light_fade_with_time(g_led_handle, CHANNEL_ID_GREEN, green * LIGHTS_LED_FULL_DUTY / 255, 0);
    MDF_ERROR_CHECK(ret < 0, ret, "iot_light_fade_with_time, ret: %d", ret);

    ret = iot_light_fade_with_time(g_led_handle, CHANNEL_ID_BLUE, blue * LIGHTS_LED_FULL_DUTY / 255, 0);
    MDF_ERROR_CHECK(ret < 0, ret, "iot_light_fade_with_time, ret: %d", ret);
...
esp_err_t led_blink_start(uint8_t red, uint8_t green, uint8_t blue, uint32_t period_ms)
{
    ...
    ret = iot_light_breath_config(g_led_handle, CHANNEL_ID_RED,
                                  red * LIGHTS_LED_FULL_DUTY / 255, period_ms);
    MDF_ERROR_CHECK(ret < 0, ESP_FAIL, "iot_light_breath_config, ret: %d", ret);
    ret = iot_light_breath_config(g_led_handle, CHANNEL_ID_GREEN,
                                  green * LIGHTS_LED_FULL_DUTY / 255, period_ms);
    MDF_ERROR_CHECK(ret < 0, ESP_FAIL, "iot_light_breath_config, ret: %d", ret);
    ret = iot_light_breath_config(g_led_handle, CHANNEL_ID_BLUE,
                                  blue * LIGHTS_LED_FULL_DUTY / 255, period_ms);
    MDF_ERROR_CHECK(ret < 0, ESP_FAIL, "iot_light_breath_config, ret: %d", ret);

    ret = iot_light_operate_start(g_led_handle, CHANNEL_ID_RED);
    MDF_ERROR_CHECK(ret < 0, ESP_FAIL, "iot_light_operate_start, ret: %d", ret);
    ret = iot_light_operate_start(g_led_handle, CHANNEL_ID_GREEN);
    MDF_ERROR_CHECK(ret < 0, ESP_FAIL, "iot_light_operate_start, ret: %d", ret);
    ret = iot_light_operate_start(g_led_handle, CHANNEL_ID_BLUE);
    MDF_ERROR_CHECK(ret < 0, ESP_FAIL, "iot_light_operate_start, ret: %d", ret);

    g_led_blink_flag = true;
    return ESP_OK;
}
...
esp_err_t led_blink_stop()
{
    ...
    if (g_led_blink_flag == false) {
        return ESP_OK;
    }

    ret = iot_light_operate_stop(g_led_handle, CHANNEL_ID_RED);
    MDF_ERROR_CHECK(ret < 0, ESP_FAIL, "iot_light_operate_stop, ret: %d", ret);

    ret = iot_light_operate_stop(g_led_handle, CHANNEL_ID_GREEN);
    MDF_ERROR_CHECK(ret < 0, ESP_FAIL, "iot_light_operate_stop, ret: %d", ret);

    ret = iot_light_operate_stop(g_led_handle, CHANNEL_ID_BLUE);
    MDF_ERROR_CHECK(ret < 0, ESP_FAIL, "iot_light_operate_stop, ret: %d", ret);
...
lights_init( void ){
    ...
    for (int i = 0; i < LIGHTS_LED_NUM; ++i) {
        g_led_handle = iot_light_create(LEDC_TIMER_0, LEDC_HIGH_SPEED_MODE, 5000, 3, LIGHTS_LED_TIMER_BIT);
        iot_light_channel_regist(g_led_handle, CHANNEL_ID_RED, LIGHTS_GPIO_LED_RED, LEDC_CHANNEL_0);
        iot_light_channel_regist(g_led_handle, CHANNEL_ID_GREEN, LIGHTS_GPIO_LED_GREEN, LEDC_CHANNEL_1);
        iot_light_channel_regist(g_led_handle, CHANNEL_ID_BLUE, LIGHTS_GPIO_LED_BLUE, LEDC_CHANNEL_2);
    }
}

/* examples
            led_set_rgb(128, 128, 0);          // yellow 
            led_set_rgb(0, 0, 0);              // off 
            led_blink_start(128, 128, 0, 2000); // yellow blink 
            led_set_rgb(128, 128, 0);           // yellow 
            led_blink_start(0, 0, 128, 3000);   // blue blink 
